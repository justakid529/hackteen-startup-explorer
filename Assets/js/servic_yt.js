...
document
  ...
  ?.addEventListener("submit", function (event) {
    ...
    if (descricao.length < 30 || descricao.length > 80) {
      alert("A descrição deve ter entre 30 e 80 caracteres.");
      return;
    }
    if (preco.length > 30) {
      alert("O preço deve ter no máximo 30 caracteres.");
      return;
    }

    const urlRegex = /^(https?:\/\/)[^\s$.?#].[^\s]*$/;
    if (!urlRegex.test(link)) {
      alert("O link deve ser uma URL válida.");
      return;
    }
    if (!urlRegex.test(logo)) {
      alert("A logo deve ser uma URL válida.");
      return;
    }
    ...
    switch (categoria) {
      case "texto":
        window.location.href = "geracaotexto.html";
        break;
      case "imagem":
      ...
      case "org":
        ...
        break;
      case "format":
        window.location.href = "formatacao.html";
        break;
    }
  });
...
document
  ...
  ?.addEventListener("submit", function (event) {
    ...
    if (descricao.length < 20 || descricao.length > 80) {
      alert("A descrição deve ter entre 20 e 80 caracteres.");
      return;
    }
    const urlRegex = /^(https?:\/\/)[^\s$.?#].[^\s]*$/;
    if (!urlRegex.test(link)) {
    ...
    if (categoria === "youtube") {
      div.innerHTML = `
        ...
        <div class="service-side">
          <iframe width="200" height="113" src="${converterParaEmbed(
            servico.link
...
document.getElementById("resetBtn")?.addEventListener("click", function () {
  const categoria =
    document.querySelector(".services-title")?.dataset.categoria;
  if (!categoria) return;
